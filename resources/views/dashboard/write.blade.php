<x-dashboard-layout title="Write a Story">
    <x-slot name="header">
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <span id="autoSaveIndicator" class="text-xs text-gray-500 hidden">
                    <svg class="inline w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    Saving...
                </span>
                <span id="autoSaveSaved" class="text-xs text-green-600 hidden">✓ Saved</span>
            </div>
            <div class="flex items-center gap-3">
                <button type="button" id="distractionFreeBtn" class="inline-flex items-center gap-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium py-2 px-3" title="Distraction-free mode (F11)">
                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/></svg>
                    Focus Mode
                </button>
    ...
    <div class="py-10" id="writeContainer">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            @if(session('success'))
                <div class="rounded-lg border border-green-200 bg-green-50 text-green-900 px-4 py-3">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <form method="POST" action="{{ route('dashboard.blog.store') }}" enctype="multipart/form-data" class="xl:col-span-2 bg-white rounded-2xl border border-gray-200 shadow-sm">
                    ...
                    <div class="p-6 border-b border-gray-100">
                        <h3 class="text-lg font-semibold text-gray-900">Compose Your Story</h3>
                        <p class="text-sm text-gray-500 mt-1">Share your thoughts with the world. Write beautifully.</p>
                    </div>

                    <div class="p-6 space-y-6">
                        <div>
                            <label for="title" class="block text-sm font-medium text-gray-700">Title</label>
                            <input id="title" name="title" type="text" required value="{{ old('title') }}" placeholder="Give your story a compelling title" class="mt-2 w-full rounded-lg border border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 py-2.5 px-3 text-lg font-semibold" />
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="tags" name="tags[]" multiple class="mt-2 w-full rounded-lg border border-gray-300 bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 py-2.5 px-3 h-[42px]">
                                    @foreach($tags as $tag)
                        ...
                        <div>
                            ...
                            <textarea id="excerpt" name="excerpt" rows="2" placeholder="A brief summary that appears in previews (recommended 160 characters)" class="mt-2 w-full rounded-lg border border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 py-2.5 px-3">{{ old('excerpt') }}</textarea>
                            <div class="mt-1 text-xs text-gray-500"><span id="excerptCount">0</span>/160 characters</div>
                        </div>

                        <div>
                            <label for="body" class="block text-sm font-medium text-gray-700 mb-2">Story Content</label>
                            ...
                            <textarea id="body" name="body" required placeholder="Write your story here... Use the toolbar above for formatting." class="w-full rounded-b-lg border border-t-0 border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 p-4 leading-7 font-serif resize-none overflow-hidden transition-all duration-300 min-h-[450px]">{{ old('body') }}</textarea>
                            <div class="mt-4 space-y-2">
                                ...
                                <div class="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden shadow-inner">
                                    <div id="wordGoalBar" data-goal="800" class="h-full bg-emerald-400 transition-all duration-500 ease-out shadow-sm" style="width:0%"></div>
                                </div>
                                <p class="text-[10px] text-gray-400 italic">Recommended length: 800+ words</p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-2">
                                <input id="featured_image" name="featured_image" type="file" accept="image/*" class="block w-full text-sm text-gray-900 file:mr-3 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer" />
                            </div>
                            <div id="featuredPreview" class="mt-3 hidden">
                                <img src="" alt="Preview" class="max-h-64 rounded-lg border border-gray-200 object-cover">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label for="published_at" class="block text-sm font-medium text-gray-700">Schedule (optional)</label>
                        ...
                        <details class="border border-gray-200 rounded-lg">
                            <summary class="px-4 py-3 cursor-pointer font-medium text-gray-700 hover:bg-gray-50">SEO Settings</summary>
                            <div class="px-4 pb-4 space-y-4">
                                <div>
                                    <label for="slug" class="block text-sm font-medium text-gray-700">URL Slug (optional)</label>
                                    <input id="slug" name="slug" type="text" placeholder="auto-generated-from-title" value="{{ old('slug') }}" class="mt-2 w-full rounded-lg border border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 py-2.5 px-3" />
                                </div>
                                <div>
                                    <label for="meta_title" class="block text-sm font-medium text-gray-700">Meta Title</label>
                                    <input id="meta_title" name="meta_title" type="text" value="{{ old('meta_title') }}" placeholder="Defaults to story title" class="mt-2 w-full rounded-lg border border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 py-2.5 px-3" />
                                </div>
                                <div>
                    ...
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-3 rounded-b-2xl">
                        ...
                        <div class="flex items-center gap-3">
                            <button type="button" id="saveDraftBtn" class="inline-flex items-center gap-2 rounded-lg bg-gray-200 hover:bg-gray-300 text-gray-800 text-sm font-medium py-2 px-4">
                                Save Draft
                            </button>
                            <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium py-2 px-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sticky top-6 h-fit">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/></svg>
                        Live Preview
                    </h3>
                    <div class="space-y-3">
                        <div class="text-2xl font-bold text-gray-900 leading-tight" id="previewTitle">Your Title</div>
                        <div class="text-sm text-gray-500" id="previewExcerpt">Your excerpt will appear here</div>
                        <div id="previewImageWrap" class="aspect-video rounded-lg border border-gray-200 bg-gray-50 overflow-hidden hidden">
                            <img id="previewImage" class="w-full h-full object-cover" alt="Preview" />
                        </div>
                        <div class="prose prose-sm max-w-none text-gray-700" id="previewBody"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        const wordGoalTarget = Number(wordGoalBar?.dataset.goal || 800);
        ...
        function updateExcerptCount() {
            const len = (excerptEl?.value || '').trim().length;
            if (excerptCount) excerptCount.textContent = Math.min(len, 160);
        }

        function updatePreview() {
            if (previewTitle && titleEl) previewTitle.textContent = titleEl.value || 'Your Title';
            if (previewExcerpt && excerptEl) previewExcerpt.textContent = excerptEl.value || 'Your excerpt will appear here';
        ...
        excerptEl?.addEventListener('input', () => { updateExcerptCount(); updatePreview(); });
        updateBodyStats();
        updateExcerptCount();
        updatePreview();
        ...
        if (fi && previewImage && previewImageWrap) {
        ...
        if (distractionFreeBtn) {
            distractionFreeBtn.addEventListener('click', () => {
                ...
                if (document.body.classList.contains('distraction-free')) {
                    writeContainer?.classList.add('max-w-4xl');
                    writeContainer?.querySelector('.xl\\:col-span-2')?.classList.remove('xl:col-span-2');
                    writeContainer?.querySelector('.xl\\:grid-cols-3')?.classList.add('xl:grid-cols-1');
                } else {
                    writeContainer?.classList.remove('max-w-4xl');
                writeContainer?.querySelector('[class*="col-span"]')?.classList.add('xl:col-span-2');
                    writeContainer?.querySelector('.xl\\:grid-cols-1')?.classList.remove('xl:grid-cols-1');
                    writeContainer?.querySelector('.xl\\:grid-cols-1')?.classList.add('xl:grid-cols-3');
                }
            });
        }

        // Auto-save functionality (localStorage fallback)
        let autoSaveTimer;
        function autoSave() {
            const data = {
                title: titleEl?.value || '',
            ...
            localStorage.setItem('blog_autosave_{{ auth()->id() }}', JSON.stringify(data));
        ...
        [titleEl, excerptEl, bodyEl].forEach(el => {
            el?.addEventListener('input', () => {
                clearTimeout(autoSaveTimer);
                document.getElementById('autoSaveIndicator')?.classList.remove('hidden');
                document.getElementById('autoSaveSaved')?.classList.add('hidden');
        ...
        window.addEventListener('load', () => {
            const saved = localStorage.getItem('blog_autosave_{{ auth()->id() }}');
            if (saved && (!titleEl?.value && !bodyEl?.value)) {
                const data = JSON.parse(saved);
                if (confirm('Recover auto-saved draft from ' + new Date(data.timestamp).toLocaleString() + '?')) {
